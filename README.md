...
- 🧮 **LaTeX 学术公式渲染（KaTeX & 离线优雅降级）**：
  - 完美支持行内公式（`$...$`）与独立块公式（`$$...$$`），集成 KaTeX 自动渲染引擎，呈现学术出版物级别的数学公式排版；
  - 智能公式字符保护机制，防止转义字符或下划线被 Markdown 解析器意外破坏；
  - 离线环境下自动以代码胶囊（Code Badge）样式优雅降级，杜绝裸露的 LaTeX 源码错乱。
- 🔄 **增量更新与智能去重**：
